...
| Test suite | PASS | `make test` passed, 153 tests / 0 failures. |
| Backend gate | PASS | `gates/backend-checks.sh` passed, 153 tests / 0 failures. |
| Full gate | PASS | `gates/checks.sh` passed, 153 tests / 0 failures. |
...
Reason: deterministic repo, packaging, and policy gates are green, but public release still requires external/operator evidence: Google secret rotation, Developer ID signing, notarization/stapling, live provider smoke tests, provider revocation/cleanup, manual accessibility checks, public privacy/legal publication, release notes, upload/distribution approval, and final operator go/no-go.
